...
## 1. Установка MND одной командой

Откройте обычный PowerShell. Права администратора не нужны:
...
- проверит Node.js 20+ и npm;
- скачает актуальную ветку `main` с GitHub и соберёт CLI;
- установит MND в `%LOCALAPPDATA%\Programs\MND`;
- создаст изолированное Python-окружение для локальной транскрибации, если найден Python 3.10+;
- создаст `%LOCALAPPDATA%\Programs\MND\bin\mnd.cmd`;
- аккуратно добавит только этот каталог в пользовательский `PATH`, не стирая остальные записи;
- сразу проверит `mnd --help`.

Команда `mnd` после этого работает из любой папки, в текущем и в новых терминалах:
...
Повторный запуск однострочной команды обновляет установленную копию. Пользовательский vault, конфигурация, проекты и ключи при этом не удаляются. Установщик не выполняет Google-аутентификацию и не изменяет сессию Antigravity.

Перед выполнением скрипт можно просмотреть:

```powershell
irm https://raw.githubusercontent.com/waters1ze/mnd/main/install.ps1
```

Для нестандартного каталога сначала скачайте установщик и передайте параметры явно:

```powershell
irm https://raw.githubusercontent.com/waters1ze/mnd/main/install.ps1 -OutFile .\install-mnd.ps1
.\install-mnd.ps1 -InstallRoot "D:\Programs\MND"
```

| Параметр установщика | Назначение |
|---|---|
| `-InstallRoot <path>` | изменить каталог установки |
| `-Repository <owner/repo>` | использовать другой fork |
| `-Branch <name>` | установить другую ветку |
| `-SkipPython` | не создавать изолированное Python-окружение |
| `-SkipPath` | создать launcher, но не менять пользовательский `PATH` |
| `-SourceArchive <zip>` | установить локальный ZIP; предназначено для проверки и разработки |

Если Node.js или Python ещё не установлены, поставьте [Node.js 20+](https://nodejs.org/) и [Python 3.10+](https://www.python.org/downloads/windows/), затем повторите ту же команду. Без Python CLI и облачная транскрибация доступны, но локальный Whisper работать не будет.

## 2. Установка Antigravity CLI
...
## 4. Самый короткий путь: папка → готовый монтаж
...
5. скрытым системным запросом передаст Groq точный каталог возможностей MND и получит список известных/неизвестных операций;
6. попросит Antigravity проверить каждый промпт; skill создаётся только если Groq явно вернул неизвестную возможность;
7. передаст ограниченный JSON-контекст выбранной модели через `agy --print --mode plan`;
8. отклонит неизвестные source ID, выход за границы, пересечения, неверный FPS или неподдерживаемые эффекты;
9. материализует требующие рендера Ч/Б и pitch-клипы встроенным FFmpeg, затем соберёт и повторно проверит FCPXML;
10. следующим проверенным запросом попросит Antigravity создать название, описание, теги и выбрать кадр;
11. проверит source ID и таймкод, затем создаст `thumbnail.jpg` через FFmpeg.
...
### Переходы, Ч/Б и обработка голоса из промпта

Эти операции можно комбинировать в обычном `--prompt` или `/prompt`:

```text
При резкой смене кадра добавь короткий плавный переход, но только если он выглядит естественно.
Когда я говорю «это было неожиданно», сделай кадр чёрно-белым.
Сделай голос немного ниже, громче на +3 дБ, убери постоянный шум и выровняй громкость.
```

- MND ставит только проверенный `cross_dissolve` и только при резкой склейке, когда у предыдущего и следующего source range хватает неиспользованных media handles. Если handles нет, остаётся чистая склейка;
- `без переходов` полностью отключает автоматические переходы;
- громкость ограничена диапазоном `-24…+12 dB`;
- voice EQ поддерживает `voice_enhance`, усиление/ослабление баса и высоких частот;
- шумоподавление и loudness записываются редактируемыми adjustment-элементами FCPXML;
- изменение высоты голоса ограничено `-4…+4` полутонами;
- Ч/Б и pitch материализуются в `MND_Export/Assets/effects/` как отдельные проверенные media clips. Timeline остаётся редактируемой, а исходники не изменяются.

### Groq → Antigravity → MND Skills

Groq получает в скрытом системном сообщении каталог реально реализованных операций и содержимое установленных MND Skills. Затем Antigravity проверяет каждый недетерминированный монтажный промпт:

1. если все операции известны, Antigravity подтверждает проверку и ничего не меняет;
2. если Groq вернул `unknownCapabilities`, Antigravity обязан подготовить декларативный рецепт;
3. MND проверяет рецепт и запрещает код, shell-команды, URL и произвольные executable-инструкции;
4. рецепт сохраняется как `Skills/<id>.md` с YAML frontmatter и сразу участвует в следующих планах;
5. терминал выводит `Скилл: <название> успешно создан`;
6. `/skills` показывает созданный skill. Статус `active` означает, что он целиком собран из реализованных операций; `instruction_only` означает, что рецепт сохранён, но движку ещё нужна отдельная реализация новой primitive-операции.

`--deterministic` по определению не вызывает ни Groq, ни Antigravity и поэтому не запускает capability audit или генерацию skills.
...
| `--deterministic` | не вызывать AI и capability audit, оставить deterministic baseline |
...
| `/skills [name]` | показать установленные MND Skills, их статус и capability mapping |
