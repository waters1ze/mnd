...
8. после зелёного статуса откройте `final-timeline.fcpxml`.
...
| `/thumbnail --full|--layers` | legacy thumbnail flow; новый `agy` не заявляет проверенный image-output contract |
...
- Новый `agy` проверенно предоставляет chat/agent orchestration, выбор модели, plan mode и workspace directories, но не документирует контракт возврата созданного image-файла. Поэтому MND не выдаёт фиктивный успех для image/thumbnail generation.
